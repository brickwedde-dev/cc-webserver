...
module.exports = {
  doLetsEncrypt: function (domains, server) {
    this.runLetsencrypt = async function runLetsencrypt () {
      this.runLetsencryptv2 (domains, server);
      if (!server) {
        process.exit();
      }
    };
  ...
  runLetsencryptv2 : async function  (domains, server) {
    ...
    if (server && domains) {
      ...
      for(var i = 0; i < domains.length; i++) {
        server.addContext(domains[i], context);
      }
    }
  },
...
//                  console.error(err);
    ...
    if (options.key) {
      if (options.key) {
        ...
        runLetsencrypt = (domains) => {
          this.runLetsencryptv2 (domains, server);
        };
        doLetsEncrypt = (domains) => {
          setInterval(() => {
            runLetsencrypt(domains);
          }, 24 * 3600 * 1000);
        };
    ...
    return { server, options, mimemapping, failcount, runLetsencrypt, doLetsEncrypt };
  },
